...
各桁は1~5のいずれかで構成されています。
4桁の数を入力すると、"場所まで合っている数字の数"、
"場所は合っていないが答えに含まれている数字の数"が
ヒントとして与えられます。
その情報を基に答えとなる4桁の数字を当てます。
'''

#各桁がそれぞれ1~5のいずれかで構成され、かつそれぞれが異なる4桁の数を生成する
def generate_answer():
  ans=[]
  count=0
  while count<4:
...
print('数当てゲームをします。各桁の数がそれぞれ異なる4桁の数を当ててください。各桁は1~5のいずれかです。\n')
...
for k in range(n):
  ...
  while True: #当たるまでループ
    ...
    if hit==4: #正解だった場合
      ...
      print('数字も場所も正しい：'+str(hit)+'\n数字は正しいが場所は違う：'+str(blow))
